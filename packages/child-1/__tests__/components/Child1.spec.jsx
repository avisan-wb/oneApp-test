...
import {
  Child1, mapDispatchToProps, mapStateToProps, loadModuleData,
} from '../../src/components/Child1';
...
describe('Child1 should render as expected', () => {
  it('module should render correct JSX', () => {
    ...
    const renderedModule = shallow(<Child1 {...props} />);
  ...
  describe('mapDispatchToProps', () => {
    it('should update the browser locale and then reload the language pack', async () => {
      ...
      expect(mockDispatch).toHaveBeenNthCalledWith(2, 'I am loading the language pack for child-1 and my fallback locale is en-US');
    });
  });
  ...
  describe('loadModuleData', () => {
    ...
    it('should load language pack for child-1 module ', async () => {
      const langPackAsyncState = await loadModuleData({ store: fakeStore });
      expect(langPackAsyncState).toBe('I am loading the language pack for child-1 and my fallback locale is en-US');
